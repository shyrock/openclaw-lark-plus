...
async function executeAuthorize(params) {
    ...
    // 0. [openclaw-lark-plus] Check user access (multi-user mode).
    const sdk = lark_client_1.LarkClient.fromAccount(account).sdk;
    try {
        await (0, owner_policy_1.assertUserAccessAllowed)(account, sdk, senderOpenId, cfg);
    }
    catch (err) {
        if (err instanceof owner_policy_1.OwnerAccessDeniedError) {
            log.warn(`unauthorized user ${senderOpenId} attempted to authorize`);
            return json({
                error: 'permission_denied',
                message: '您不在授权用户列表中，无法发起授权。请联系管理员将您添加到 authorizedUsers。',
            });
        }
        throw err;
    }
