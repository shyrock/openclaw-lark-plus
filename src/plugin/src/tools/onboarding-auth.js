...
async function triggerOnboarding(params) {
    ...
    const { appId } = acct;
    // 1. [openclaw-lark-plus] Check user against authorizedUsers config.
    //    Replaces the original owner-only check to support multi-user onboarding.
    ...
    if (authorizedUsers && authorizedUsers !== '*') {
        if (!Array.isArray(authorizedUsers) || !authorizedUsers.includes(userOpenId)) {
            log.info(`user ${userOpenId} not in authorizedUsers, skipping onboarding`);
            return;
        }
    }
    log.info(`user ${userOpenId} authorized, starting OAuth onboarding`);
