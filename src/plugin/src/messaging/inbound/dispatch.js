...
const user_workspace_1 = require("../../core/user-workspace.js");
...
async function dispatchToAgent(params) {
    ...
    // [openclaw-lark-plus] Per-user system prompt takes precedence over group prompt
    const userWs = (0, user_workspace_1.resolveUserWorkspace)(dc.accountScopedCfg, dc.ctx.senderId);
    const groupSystemPrompt = userWs?.systemPrompt?.trim()
        || (dc.isGroup
    ...
    // Resolve skill filter: per-user > per-group > default "*"
    // [openclaw-lark-plus] Per-user skill filter
    const userSkillFilter = userWs?.skills;
    const groupSkillFilter = dc.isGroup ? (params.groupConfig?.skills ?? params.defaultGroupConfig?.skills) : undefined;
    const skillFilter = userSkillFilter ?? groupSkillFilter;
    if (isCommand) {
